...
def main(grid: Grid, context: Context) -> None:
    ...
    # Track metrics
    eval_interval = 10  # Evaluate the model every 10 rounds
    ...
    for i in range(1, num_rounds + 1):
        ...
        if i % eval_interval == 0 or i == num_rounds:
            accuracy, sensitivity, specificity = evaluate_head_model(head, embeddings, labels)
            log(INFO, f"Round {i}, Loss: {loss.item():.4f}, Accuracy: {accuracy:.2f}%, Se: {sensitivity:.2f}%, Sp: {specificity:.2f}%")
    ...
    for (round_num, accuracy), (_, loss_value), (_, sensitivity), (_, specificity) in zip(accuracies, losses, sensitivities, specificities):
        log(
            INFO,
