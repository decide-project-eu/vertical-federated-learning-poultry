from logging import INFO

import torch
from datasets import load_dataset
from flwr.app import Array, ArrayRecord, ConfigRecord, Context, Message, RecordDict
from flwr.common import log
from flwr.serverapp import Grid, ServerApp

from vertical_fl_own.task import FEATURE_COLUMNS, ServerModel, evaluate_head_model

# Create ServerApp
app = ServerApp()


@app.main()
def main(grid: Grid, context: Context) -> None:
    """Main entry point for the ServerApp."""
    # Read run config
    num_rounds: int = context.run_config["num-server-rounds"]
    # Check if number of features are correct
    feature_splits: str = context.run_config["feature-splits"]
    in_feature_dim_clientapp = [int(dim) for dim in feature_splits.split(",")]
    print("FEATURE_COLUMNS:", FEATURE_COLUMNS)
    print("len(FEATURE_COLUMNS):", len(FEATURE_COLUMNS))
    if sum(in_feature_dim_clientapp) != len(FEATURE_COLUMNS):
        raise ValueError(
            "The sum of feature splits must equal the total number of features "
            f"(got {sum(in_feature_dim_clientapp)} vs. {len(FEATURE_COLUMNS)})."
        )
    out_feature_dim_clientapp: int = context.run_config["out-feature-dim-clientapp"]

     # ---- Early stopping configuration ----
    patience = context.run_config.get("early-stopping-patience")
    min_delta = context.run_config.get("early-stopping-min-delta")
    best_loss = float("inf")
    early_stop_counter = 0

    # Get dataset (training set!)
    dataset = load_dataset(
        "csv",
        data_files="C:/Users/4243692/vertical-fl/vertical-fl-own/full_dataset.csv",
        split="train" # comes from HuggingFace, needs to be 'train' for single csv.
    )
    labels = dataset["AntibioticsAfterWeek1"]
    labels = torch.tensor(labels).float().unsqueeze(1)
    
    # Create train-val split
    N = len(labels)
    perm = torch.randperm(N)
    train_idx = perm[: int(0.8 * N)]
    val_idx = perm[int(0.8 * N):]

    train_labels = labels[train_idx]
    val_labels = labels[val_idx]

    # Serverapp model
    head = None
    optimizer = None
    #criterion = torch.nn.BCELoss()
    criterion = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor(4.5))    # hardcoded, change!

    # Track metrics
    eval_interval = 5  # Evaluate the model every 10 rounds
    accuracies: list[tuple[int, float]] = []
    val_losses: list[tuple[int, float]] = []
    sensitivities: list[tuple[int, float]] = []
    specificities: list[tuple[int, float]] = []

    for i in range(1, num_rounds + 1):
        log(INFO, "")
        log(INFO, f"--- ServerApp Round {i} / {num_rounds} ---")

        node_ids = list(grid.get_node_ids())

        if len(node_ids) != len(in_feature_dim_clientapp):
            raise ValueError(
                "The number of feature splits must equal the number of nodes "
                f"(got {len(in_feature_dim_clientapp)} vs. {len(node_ids)})."
            )

        if head is None:
            # The server model's input size is determined by the number of clients
            # and the output feature dimension of each embedding produced by the clients
            head = ServerModel(input_size = out_feature_dim_clientapp * len(node_ids))
            optimizer = torch.optim.Adam(head.parameters(), lr=0.01)

        # 1. Get embeddings from all clients
        embeddings, node_pos_mapping = get_remote_embeddings(
            grid = grid,
            node_ids = node_ids,
            indices = train_idx,
            embedding_dim = out_feature_dim_clientapp,
        )

        # 2. Complete forward pass and compute loss
        optimizer.zero_grad()  # Clear gradients before backward pass
        output = head(embeddings)
        loss = criterion(output, train_labels)                              
        loss.backward()

        # 4. Extract gradients w.r.t. embeddings
        embeddings_grad = embeddings.grad.split(
            [out_feature_dim_clientapp] * len(node_ids), dim=1
        )

        # Update the head model
        optimizer.step()

        # Validation step
        with torch.no_grad():
            val_embeddings, _ = get_remote_embeddings(
                grid,
                node_ids,
                val_idx,
                out_feature_dim_clientapp,
            )

            val_output = head(val_embeddings)
            val_loss = criterion(val_output, val_labels)

        # Early stopping
        current_loss = val_loss.item()                                   

        if best_loss - current_loss > min_delta:
            best_loss = current_loss
            early_stop_counter = 0
        else:
            early_stop_counter += 1

        if early_stop_counter >= patience:
            log(INFO, "Early stopping triggered.")
            break

        # 3. Compute accuracy using updated head model                                                                              
        if i % eval_interval == 0 or i == num_rounds:
            accuracy, sensitivity, specificity = evaluate_head_model(head, val_embeddings, val_labels)
            log(
                INFO, 
                f"Round {i}, Train loss: {loss.item():.4f}, Val loss: {val_loss.item():.4f}, Accuracy: {accuracy:.2f}%, Se: {sensitivity:.2f}%, Sp: {specificity:.2f}%"
            )

            accuracies.append((i, accuracy))
            val_losses.append((i, val_loss.item()))
            sensitivities.append((i, sensitivity))
            specificities.append((i, specificity))

        # 5. Send gradients to clients
        send_gradients_to_clients(grid, node_pos_mapping, embeddings_grad, train_idx)

    # Log final results
    log(INFO, "")
    log(INFO, "=== Final Results ===")
    for (round_num, accuracy), (_, loss_value), (_, sensitivity), (_, specificity) in zip(accuracies, val_losses, sensitivities, specificities):
        log(
            INFO,
            f"Round {round_num} -> Loss: {loss_value:.4f} | Accuracy: {accuracy:.2f}% | Se: {sensitivity:.2f}%, Sp: {specificity:.2f}%" ,
        )


def send_gradients_to_clients(
    grid: Grid,
    node_pos_mapping: dict[int, int],
    embeddings_grad: list[torch.Tensor],
    indices  #train_idx or val_idx
) -> None:
    """Send gradients to clients."""

    # Create messages that target method in ClientApp with
    # @app.train("apply_gradients") decorator
    messages = []
    for node_id, pos in node_pos_mapping.items():
        arrc = ArrayRecord({"local-gradients": Array(embeddings_grad[pos].numpy())})
        conf = ConfigRecord({"indices": indices.tolist()})
        message = Message(
            content = RecordDict({"gradients": arrc, "config": conf}),
            message_type = "train.apply_gradients",
            dst_node_id = node_id,
        )
        messages.append(message)

    # Send messages, but don't wait for results (no replies expected)
    log(INFO, "Sending gradients to %s nodes...", len(messages))
    grid.push_messages(messages)


def get_remote_embeddings(
    grid: Grid,
    node_ids: list[str],
    indices,
    embedding_dim: int,
) -> tuple[torch.Tensor, dict[int, int]]:
    """Get embeddings from sampled remote nodes."""

    # Create messages that target method in ClientApp with
    # @app.query("generate_embeddings") decorator
    messages = []
    for node_id in node_ids:  # one message for each node
        message = Message(
            content=RecordDict({
                "config": ConfigRecord({
                    "indices": indices.tolist()
                })
            }),
            message_type="query.generate_embeddings",
            dst_node_id=node_id,
        )
        messages.append(message)

    # Send messages and wait for all results
    log(INFO, "Requesting embeddings from %s nodes...", len(messages))
    replies = grid.send_and_receive(messages)
    log(INFO, "\tReceived %s/%s results", len(replies), len(messages))

    embeddings = torch.zeros((len(indices), embedding_dim * len(node_ids)))

    # Convert all embeddings back to pytorch tensors
    # and place them in the corresponding feature segment
    node_pos_mapping: dict[int, int] = {}
    for reply in replies:
        if reply.has_content():
            arr = reply.content["arrays"]["embedding"]
            embd = torch.from_numpy(arr.numpy())
            ids_arr = reply.content["arrays"]["ids"]
            returned_ids = torch.from_numpy(ids_arr.numpy())

            # Check if IDs are correct
            if not torch.equal(returned_ids, indices):
                raise RuntimeError(
                    f"Client {reply.metadata.src_node_id} returned "
                    "embeddings in wrong order!"
                )

            pos = reply.content["config"]["pos"]
            node_pos_mapping[reply.metadata.src_node_id] = pos
            embeddings[:, pos * embedding_dim : (pos + 1) * embedding_dim] = embd

    return embeddings.requires_grad_(), node_pos_mapping



#### Evaluate on test set (NON-FEDERATED) #######

"""
dataset = load_dataset(
        "csv",
        data_files="C:/Users/4243692/vertical-fl/vertical-fl-own/test_split.csv",
        split="test_split"
    )
...
Choose: non-federated (after model training), then I need to save the preprocessing per client. Or with server and clients similar to train-validation split, only then do this once at the end.
"""